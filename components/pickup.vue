<template>
  <vue-autosuggest
    ...
    :input-props="{id:'autosuggest__input', placeholder:$store.state.pickup_address,
                   autocomplete:'off'}"
    @input="getAddresses"
    @selected="selected"
    class="form-control"
  >
    <template slot-scope="{suggestion}">
      ...
      <span v-if="$store.state.booking_type === 'FromAirport'" class="my-suggestion-item">{{ suggestion.item.name }}</span>
...
export default {
  ...
  mounted() {
    if (this.$store.state.booking_type === 'FromAirport') {
  ...
  methods: {
    ...
    selected(suggestion) {
      this.$emit('updatePickup', suggestion.item)
